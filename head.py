...
from mmcv.cnn import Scale, is_norm
...
class DecoupleHead_v2(ATSSHead):
    ...
    def _init_layers(self):
        ...
        self.cls_convs.append(
            DyReLU(channels=self.in_channels, act_cfg=self.task_act_cfg)
        )
    ...
    def get_targets(
        ...
        # 前面把每张图片所有尺度的预测合在一起做正负样本的分配、采样等工作，现在再把结果拆分回不同的尺度
    ...
    def _get_targets_single(
        ...
        anchors = flat_anchors[inside_flags, :]  # 在真实图片范围内的anchors
        ...
        labels = anchors.new_full(
            (num_valid_anchors,),  # 初始化每个有效anchor对应的标签目标为目标类别数
            self.num_classes,
            dtype=torch.long,
        )
...
class SepDecoupleHead_v2(DecoupleHead_v2):
    ...
    def _init_layers(self) -> None:
        ...
        if self.share_conv:
            for n in range(len(self.prior_generator.strides)):
                for i in range(self.stacked_convs):
                    self.cls_convs[n][1+i].dcn_v3 = self.cls_convs[0][1+i].dcn_v3
                    self.cls_convs[n][1+i].feed_forward = self.cls_convs[0][1+i].feed_forward
                    self.reg_convs[n][1+i].dcn_v3 = self.reg_convs[0][1+i].dcn_v3
                    self.reg_convs[n][1+i].feed_forward = self.reg_convs[0][1+i].feed_forward
